...
void EXTI9_5_IRQHandler(void)
{
  if(EXTI_GetITStatus(EXTI_Line8) != RESET)
  {
    ...
    PPM_Time=10000*TIME_ISR_CNT+TIM2->CNT/2;//us
   ...
   //PPM解析开始
    ...
    //}
      //对应通道写入缓冲区
      if(PPM_Time_Delta>=900&& PPM_Time_Delta<=2100)
      {
      PPM_Sample_Cnt++;
      PPM_Databuf[PPM_Sample_Cnt-1]=PPM_Time_Delta;
      //单次解析结束
      if(PPM_Sample_Cnt>=8)  PPM_Is_Okay=0;
      }
      else  PPM_Is_Okay=0;
    }
    if(PPM_Is_Okay==0
       &&PPM_Time_Delta>4000)//帧结束电平至少2ms=2000us，为了兼容非标PPM，如果数据会偶尔跳变，尝试加大此值
    {
      PPM_Is_Okay=1;
      PPM_Sample_Cnt=0;
    }
...
//系统运行时间获取，单位us
   ...
   //PPM解析开始
    if(PPM_Is_Okay==1)
    {
      ...
      {
          if(PPM_Time_Delta>=2500)//帧结束电平至少2ms=2000us
          {
              memcpy( PPM_Databuf,PPM_buf,PPM_Sample_Cnt*sizeof(uint16));
              PPM_Is_Okay = 1;
              PPM_Sample_Cnt=0;
