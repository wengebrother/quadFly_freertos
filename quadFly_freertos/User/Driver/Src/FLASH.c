...
volatile FLASH_Status RC_FLASHStatus = FLASH_COMPLETE;      //Flash操作状态变量
                         ...
                         Vector_RC *ch7)
{
        uint32_t Buf[16]={0};
        uint16_t i=0;
        Buf[0]=*(uint32_t *)(&ch0->min);//把内存里面这四个字节写入到Flash
...
volatile FLASH_Status ESC_FLASHStatus = FLASH_COMPLETE;      //Flash操作状态变量
void WriteFlash_ESC(uint32_t WriteAddress,uint32_t WriteData)
{
  ...
  {
      ESC_FLASHStatus = FLASH_ProgramWord(ESC_STARTADDR + WriteAddress, WriteData);    //flash.c 中API函数
  }
  FLASH_LockBank1();
}
