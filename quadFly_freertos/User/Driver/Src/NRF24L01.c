...
{
  RC_Buttom=rc_date[2].min;
  RC_Top=rc_date[2].max;
  RC_Middle=rc_date[2].middle;
  RC_Deadband=rc_date[2].deadband;
...
/****************PPM接收*****************/
 ...
 //if(PPM_Databuf[0]<=RC_Deadzone_Buttom)  Roll_Control=(RC_Deadzone_Buttom-PPM_Databuf[0])*Pit_Rol_Max/(RC_Deadzone_Buttom-RC_Buttom);//最大行程控制量+-45
 //else if(PPM_Databuf[0]>=RC_Deadzone_Top)  Roll_Control=(RC_Deadzone_Top-PPM_Databuf[0])*Pit_Rol_Max/(RC_Top-RC_Deadzone_Top);
 ...
 //if(PPM_Databuf[1]<=RC_Deadzone_Buttom)  Pitch_Control=(RC_Deadzone_Buttom-PPM_Databuf[1])*Pit_Rol_Max/(RC_Deadzone_Buttom-RC_Buttom);//最大行程控制量+-45
 //else if(PPM_Databuf[1]>=RC_Deadzone_Top)  Pitch_Control=(RC_Deadzone_Top-PPM_Databuf[1])*Pit_Rol_Max/(RC_Top-RC_Deadzone_Top);
 ...
 //Temp_RC=(PPM_Databuf[2]-(RC_Buttom+Buttom_Safe_Deadband));//为了安全，油门杆低位死区为Buttom_Safe_Deadband
 ...
 //if(PPM_Databuf[3]<=RC_Deadzone_Buttom)  Yaw_Control=-(PPM_Databuf[3]-RC_Deadzone_Buttom)*Yaw_Max/(RC_Deadzone_Buttom-RC_Buttom);//偏航最大行程控制量+-150
 //else if(PPM_Databuf[3]>=RC_Deadzone_Top)  Yaw_Control=-(PPM_Databuf[3]-RC_Deadzone_Top)*Yaw_Max/(RC_Top-RC_Deadzone_Top);
 ...
 ***************************************************************/
              ...
              &&Check_Calibration_Flag()==0x00)
  {
      ...
      Unlock_Makesure_Cnt=0;
      Forced_Lock_Makesure_Cnt=0;
      Bling_Set(&Light_1,5000,2000,0.9,0,GPIOC,GPIO_Pin_4,0);
      Bling_Set(&Light_2,5000,2000,0.5,0,GPIOC,GPIO_Pin_5,0);
      Bling_Set(&Light_3,5000,2000,0.1,0,GPIOC,GPIO_Pin_10,0);
      Bling_Mode=0;
      Page_Number=0;//OLED恢复首页
      Reset_Mag_Calibartion(1);
      Reset_Accel_Calibartion(1);
  ...
  if(Auto_Relock_Flag==1)
  {
            ...
            &&Auto_ReLock_Cnt>0)//自动上锁计数器
   {
      ...
      Auto_ReLock_Cnt=0;
      Bling_Set(&Light_1,5000,2000,0.3,0,GPIOC,GPIO_Pin_4,0);
      Bling_Set(&Light_2,5000,2000,0.3,0,GPIOC,GPIO_Pin_5,0);
      Bling_Set(&Light_3,5000,2000,0.3,0,GPIOC,GPIO_Pin_10,0);
      Bling_Mode=0;
   }
  }
}
