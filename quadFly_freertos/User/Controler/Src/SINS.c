...
//#define High_Delay_Cnt  5 //20ms
    ...
    uint16 High_Delay_Cnt=10;
#endif
#ifdef IMU_BOARD_NC686
    uint16 High_Delay_Cnt=20;//100ms
#endif
...
void Observation_Tradeoff(uint8_t HC_SR04_Enable)
{
  ...
  if(HC_SR04_Enable==0)//无超声波参与
  {
    ...
    #endif
    #ifdef  IMU_BOARD_NC683
        Altitude_Estimate=FBM320_Filter_high;//高度观测量
    #endif
    observation_flag=1;
  }
  else//有超声波参与
  {    
    ...
    {
        Altitude_Estimate=HC_SR04_Distance;
        observation_flag=2;
    }
    else
    {
    ...
    if(observation_flag==2&&last_observation_flag==1)//气压计切超声波
    {
       ...
       for(Cnt=Num-1;Cnt>0;Cnt--){NamelessQuad.Pos_History[_YAW][Cnt]=HC_SR04_Distance;}
...
float R[2]={5.0e-4f,3.0e-4f};//{5.0e-4f,3.0e-4f}
float Q=50;//50
                    ...
                    uint8_t update_flag)
{
        ...
        //float x1;
        //NamelessQuad1.Acceleration[_YAW]=System_drive;
        //x1=Altitude_Estimate-NamelessQuad1.Position[_YAW];      
        //NamelessQuad1.Position[_YAW]+=NamelessQuad1.Speed[_YAW]*Delta_T+0.5*NamelessQuad1.Acceleration[_YAW]*Delta_T*Delta_T+k[0]*x1;
        //NamelessQuad1.Speed[_YAW]+=NamelessQuad1.Acceleration[_YAW]*Delta_T+k[1]*x1;
...
else
{
  NamelessQuad.Position[_PITCH] +=NamelessQuad.Speed[_PITCH]*Horizontal_Delta_T
                                +((NamelessQuad.Acceleration[_PITCH])*Horizontal_Delta_T*Horizontal_Delta_T)/2.0;
  ...
  NamelessQuad.Position[_ROLL] +=NamelessQuad.Speed[_ROLL]*Horizontal_Delta_T
                                +((NamelessQuad.Acceleration[_ROLL])*Horizontal_Delta_T*Horizontal_Delta_T)/2.0;
