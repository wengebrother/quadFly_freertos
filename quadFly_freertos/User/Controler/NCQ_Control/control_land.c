      ...
      /*******************************竖直加速度控制器结束******************************************************************/
            if(YAW_Pos_Control_Accel_Disable==0)
          {
            Throttle=Int_Sort(//Throttle_Hover//悬停油门
                    High_Hold_Throttle+Total_Controller.High_Acce_Control.Control_OutPut);//油门来源于高度加速度控制器输出
          }
          else
          {
            Throttle=Int_Sort(//Throttle_Hover//悬停油门
                    High_Hold_Throttle+Total_Controller.High_Speed_Control.Control_OutPut);//油门来源于高度速度控制器输出
          }
      /*****************************************高度控制器结束，给定油门控制量***********************************************************/
...
void landon_earth_check(void)//自检触地进入怠速模式
{
  ...
  //油门控制处于较低行程：//1、姿态模式下，油门杆处于低位
   ...
   if(Throttle_Output<=Minimal_Thrust_Threshold
         &&Gyro_Length<=20.0//触地后无旋转，合角速度小于20deg/s
          &&ABS(NamelessQuad.Speed[_YAW])<=25.0//惯导竖直轴速度+-25cm/s
            )
     Landon_Earth_Cnt++;
